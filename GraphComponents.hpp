...
inline sf::Vector2f movePoint(const sf::Vector2f& position, double angle, double distance)
{
    auto angleRad = angle * std::acos(-1.0) / 180.0f;
    double x = position.x + distance * std::cos(angleRad);
    double y = position.y + distance * std::sin(angleRad);
    return sf::Vector2f(x, y);
}

inline double calculateAngle(const sf::Vector2f& pos1, const sf::Vector2f& pos2)
{
